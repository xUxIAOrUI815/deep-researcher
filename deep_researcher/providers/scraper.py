...
class SmartScraper:
    ...
    async def _fetch_with_playwright(self, url: str) -> ScrapedDocument:
        try:
    ...
    async def _level2_semantic_filter(
        ...
        if score < 0.5:
            print("[SemanticFilter] Score < 0.5, URL Discarded")
            return False, score
    ...
    ) -> tuple[ScrapedDocument, DenoiseStats]:
        stats = DenoiseStats()
    ...
    async def scrape_batch_with_denoise(
        ...
        query: str,
        force_playwright: bool = False
    ) -> tuple[List[ScrapedDocument], List[DenoiseStats]]:
        tasks = [
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                scraped_data.append(ScrapedDocument(
                    url=urls[i],
                    markdown="",
                    title="",
    ...
    async def _fetch_with_jina(self, url: str) -> ScrapedDocument:
